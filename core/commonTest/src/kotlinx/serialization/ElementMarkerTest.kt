...
class ElementMarkerTest {
    private class TestMarker(descriptor: SerialDescriptor, val predicate: (SerialDescriptor, Int) -> Boolean = { _, _ -> true }) :
        ElementMarker(descriptor) {

        override fun isPoppedElement(descriptor: SerialDescriptor, index: Int): Boolean =
            predicate(descriptor, index)
    }
    ...
    fun testNothingWasRead() {
        ...
        val reader = TestMarker(descriptor)

        for (i in 0 until size) {
            assertEquals(i, reader.popUnmarkedIndex())
        }
        assertEquals(CompositeDecoder.DECODE_DONE, reader.popUnmarkedIndex())
    }
    ...
    fun testFilteredRead() {
        ...
        val descriptor = createClassDescriptor(size)
        val reader = TestMarker(descriptor, predicate)
